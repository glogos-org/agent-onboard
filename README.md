...
## Current release: target handoff preview product gate

The current release adds the public target handoff preview product surface: `agent-onboard target handoff --preview`, `agent-onboard target handoff --json`, and `agent-onboard target handoff --text`. The command composes bounded target inventory, known memory/handoff surface presence, and target work-item summary into a compact next-session handoff. It does not import file contents, admit or close work items, synthesize a next id, write ledgers, mutate Git, install dependencies, run managed project commands, publish, or use network access. Handoff preview output is evidence and orientation only; it cannot grant authority.
