...
## Current release: contributor admission dry-run product gate

The current release adds the public contributor admission dry-run product surface: `agent-onboard contributor --admission-dry-run`, `agent-onboard contributor --admission-dry-run --json`, and `agent-onboard contributor --admission-dry-run --text`. The command previews actor identity, provenance, and AI-assistance attribution metadata before any authoritative admission. It may report `Assisted-by` trailer expectations and missing admission evidence, but it does not create canonical contributor records, write ledgers, admit claims, call GitHub, scan arbitrary target state, install dependencies, mutate Git, publish, or use network access.
