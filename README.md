...
Current release: `release --clean-inventory` and `release --clean-check` seed `P1S3M6 — Public clean and compaction milestone` with a read-only source surface baseline. The baseline records file counts, package projection counts, `.agent-onboard` growth, README size, keyword sprawl, work-item ledger size, and compaction candidates before any deletion, movement, archival, or compaction write. The command does not mutate work items, claims, Git, dependencies, build/test/deploy state, publication state, registry state, network state, or package-manager state.
...
## Public clean and compaction baseline

Use these commands to inspect and validate the M6 clean baseline before any compaction write:
...
This baseline closes the M5 target-policy-adapter line and opens `P1S3M6 — Public clean and compaction milestone`. It reports the current source surface, `.agent-onboard` growth, package projection size, README size, keyword count, work-item ledger size, and compaction candidates. It is intentionally no-write: future deletion, archival, movement, or compaction requires a separate admitted work item.
