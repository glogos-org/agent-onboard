...
## Current release: target handoff governance budget summary product gate

The current release adds stable handoff readiness reason codes to `target handoff --json|--text`. Handoff readiness now includes `reason_codes` and structured `reasons[]` entries with severity, summary, and a suggested next command, so budget, drift, missing-entrypoint, and open-work-item states are explainable without importing file contents or treating the preview as authority. The previous release added governance budget state to `target handoff --json|--text`: handoff composes the compact governance budget check, reports `governance_budget_summary`, blocks next-agent readiness when the budget check is blocked or over contract, and still inlines no raw work-items file, claims ledger, planned index payload, memory content, source evidence, or private provider state. `target governance --budget-check --json|--text` remains the compact no-write target scan for `.agent-onboard/work-items.index.json` and `.agent-onboard/claims.index.json` against the public 4096-byte per-index and 8192-byte combined governance budget. `--budget-contract` remains the no-scan policy surface; `--materialize-dry-run` remains the exact payload planning surface; explicit `target governance --materialize --write --force` remains the only command that may write allowlisted governance index paths with compare-before-write.
