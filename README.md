...
Current release: `release --clean-catalog` and `release --clean-catalog-check` classify the `P1S3M6 — Public clean and compaction milestone` candidates before any compaction write. The catalog sits on top of the read-only `release --clean-inventory`/`--clean-check` baseline and marks README history, closed gate artifacts, package keyword taxonomy, work-item ledger growth, identity indexes, and runtime/test monoliths as future-admission surfaces. The command does not delete, move, archive, rewrite, compact, mutate work items or claims, run Git/dependency/build/test/deploy/package-manager commands, publish, touch registry/network state, or authorize any cleanup write.
...
The baseline reports the current source surface, `.agent-onboard` growth, package projection size, README size, keyword count, work-item ledger size, and compaction candidates. The catalog then classifies each candidate surface into a preserve-first disposition: README first-read/history split, closed gate evidence compaction, package keyword taxonomy reduction, work-item ledger archive/index split, identity index refresh discipline, and runtime/test module extraction. Both surfaces are intentionally no-write: future deletion, archival, movement, taxonomy reduction, or source extraction requires a separate admitted work item that names the exact catalog surface id and preserves a recovery or replay path.
