...
npx agent-onboard release --readme-apply
npx agent-onboard release --readme-apply-check
```

The baseline reports the current source surface, `.agent-onboard` growth, package projection size, README size, keyword count, work-item ledger size, and compaction candidates. The catalog then classifies each candidate surface into a preserve-first disposition: README first-read/history split, closed gate evidence compaction, package keyword taxonomy reduction, work-item ledger archive/index split, identity index refresh discipline, and runtime/test module extraction. The README history split is now applied: live README keeps install, quickstart, current command surface, and safety boundary text; historical release prose is archived in `docs/release-history.md`; `.agent-onboard/readme-history.index.json` records recovery digests and source ranges. `release --readme-plan-check`, `release --readme-dry-run-check`, and `release --readme-apply-check` verify the applied state without deleting, moving, publishing, installing dependencies, mutating Git, or changing the npm package projection. Future deletion, movement, source extraction, or unrelated compaction still requires a separate admitted work item that names the exact catalog surface id and preserves a recovery or replay path.
...
Historical release prose has been archived in `docs/release-history.md`. The live README keeps install, quickstart, current command surface, and no-mutation boundary material as the first-read surface.
