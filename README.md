...
Current release: this line continues runtime composer decomposition by extracting modular runtime inclusion, packaged router port inclusion, thin entrypoint cutover, and router command-adapter delegation plan/check logic into `cli/agent_onboard/domains/architecture/services/runtime/public-router-cutover-service.js`. `runtime-composer.js` remains the compatibility composer and public export surface, but it no longer owns the `architecture --module-inclusion`, `architecture --module-inclusion-check`, `architecture --packaged-router-port`, `architecture --packaged-router-port-check`, `architecture --thin-entrypoint-rehearsal`, `architecture --thin-entrypoint-rehearsal-check`, `architecture --thin-entrypoint-cutover`, `architecture --thin-entrypoint-cutover-check`, `architecture --router-adapter-delegation`, or `architecture --router-adapter-delegation-check` contracts. The surface remains read-only and no-install/no-network/no-Git; no SQLite, Lightning Memory-Mapped Database, MDBX, dependency addition, registry mutation, or package publish is admitted.
