...
function main() {
  ...
  if (artifact.schema !== 'agent-onboard-public-router-cutover-service-extraction-001') failures.push('artifact schema mismatch');
  if (artifact.work_item_id !== 'P1S3M7W14') failures.push('artifact work_item_id must be P1S3M7W14');
  if (artifact.version !== '0.0.197') failures.push('artifact version must record W14 closure version 0.0.197');
  ...
  if (artifact.reduction.runtime_composer_after.lines !== MAX_COMPOSER_LINES) failures.push('artifact runtime_composer_after.lines must record W14 ratchet baseline');
  if (artifact.reduction.runtime_composer_after.bytes !== MAX_COMPOSER_BYTES) failures.push('artifact runtime_composer_after.bytes must record W14 ratchet baseline');
  if (!composer.includes('createPublicRouterCutoverService')) failures.push(`${COMPOSER_REL} must compose the router cutover service`);
  for (const name of [
  ...
  if (!serviceText.includes('function createPublicRouterCutoverService')) failures.push(`${SERVICE_REL} must export the service factory`);
  for (const token of [
  ...
  const result = {
    schema: 'agent-onboard-public-router-cutover-service-extraction-check-001',
    status: failures.length === 0 ? 'ok' : 'error',
    package_name: pkg.name,
    version: pkg.version,
    work_item_id: 'P1S3M7W14',
    composer: composerMetric,
    extracted_router_cutover_service: serviceMetric,
    smoke_count: 8,
    boundary: {
      writes_files: false,
      mutates_git: false,
