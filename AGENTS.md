...
`P1S3M6` is the current public clean and compaction milestone. Start with `agent-onboard release --clean-inventory`, `agent-onboard release --clean-check`, `agent-onboard release --clean-catalog`, `agent-onboard release --clean-catalog-check`, `agent-onboard release --keyword-taxonomy-check`, `agent-onboard release --readme-plan-check`, `agent-onboard release --readme-dry-run-check`, and `agent-onboard release --readme-apply-check`. The baseline/catalog commands remain read-only; the keyword taxonomy gate is the admitted package metadata compaction surface; and the README history split is now applied with `docs/release-history.md` plus `.agent-onboard/readme-history.index.json` as the recovery/index path while `README.md` remains the live first-read surface. These verification commands do not authorize deletion, movement outside the admitted README split, source extraction, package publication, dependency installation, Git mutation, claim mutation, or unrelated work-item mutation. Future compaction writes require a separate admitted work item, the exact catalog surface id, a preserved recovery/replay path, and explicit owner authorization.
