...
```sh
npx agent-onboard@0.0.21 target onboarding --plan
```

Validate the public release contract and source/package surface before any package publish handoff:

```sh
npx agent-onboard@0.0.21 release --check
```

Inspect the normalized release contract:

```sh
npx agent-onboard@0.0.21 release --contract
```

Inspect the release fixture matrix:

```sh
npx agent-onboard@0.0.21 release --fixture
```

Run the installed package parity smoke:

```sh
npx agent-onboard@0.0.21 release --parity-smoke
```
...
```sh
npx agent-onboard@0.0.21 work-items --close --dry-run --id <public-work-item-id> --actor <agent-or-human-name> --summary <summary>
```
