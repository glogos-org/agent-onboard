...
```sh
npx agent-onboard@0.0.28 target onboarding --plan
npx agent-onboard@0.0.28 target onboarding --fixture
npx agent-onboard@0.0.28 target onboarding --trial
```
...
```sh
npx agent-onboard@0.0.28 target onboarding --write
```
...
```sh
npx agent-onboard@0.0.28 architecture --map
npx agent-onboard@0.0.28 architecture --check
```

Validate the public release contract and source/package surface before any package publish handoff:

```sh
npx agent-onboard@0.0.28 release --check
```

Inspect the normalized release contract:

```sh
npx agent-onboard@0.0.28 release --contract
```

Inspect the release fixture matrix:

```sh
npx agent-onboard@0.0.28 release --fixture
```

Run the installed package parity smoke:

```sh
npx agent-onboard@0.0.28 release --parity-smoke
```

Run the target onboarding installed-package smoke before publish handoff:

```sh
npx agent-onboard@0.0.28 release --target-onboarding-smoke
```

Inspect the post-publish verification handoff before publishing:

```sh
npx agent-onboard@0.0.28 release --post-publish-handoff
```

Run the published package acceptance rehearsal before publish, then the version-pinned acceptance check after publish:

```sh
npx agent-onboard@0.0.28 release --published-acceptance
npx agent-onboard@0.0.28 release --real-target-trial
```
...
```sh
npx agent-onboard@0.0.28 work-items --close --dry-run --id <public-work-item-id> --actor <agent-or-human-name> --summary <summary>
```
