...
const EXPECTED_RELEASE_LINE = 'public_source_module_extraction_first_slice_gate';
...

{
  ...
  assert.strictEqual(output.contract_schema, 'agent-onboard-public-release-contract-021');
...

{
  ...
  assert.strictEqual(output.version, EXPECTED_VERSION);
  assert.strictEqual(output.release_line, EXPECTED_RELEASE_LINE);
  assert.strictEqual(output.contract.schema, 'agent-onboard-public-release-contract-021');
...

{
  ...
  assert.strictEqual(output.release_line, EXPECTED_RELEASE_LINE);
  assert.strictEqual(output.contract_schema, 'agent-onboard-public-release-contract-021');
  assert.strictEqual(output.fixture_matrix.schema, 'agent-onboard-public-release-fixture-matrix-015');
...

{
  ...
  assert.strictEqual(output.map.public_source_shape.physical_domain_split_status, 'source_module_first_slice_applied_for_core_source_only');
...

{
  ...
  assert.ok(output.source_work_items_ledger.open_work_items.some((item) => item.title === 'Public source module extraction bundle parity gate'));
...

{
  ...
  assert.strictEqual(s3m1w13.status, 'open');
...

{
  ...
  assert.strictEqual(cli.PUBLIC_RELEASE_FIXTURE_MATRIX.schema, 'agent-onboard-public-release-fixture-matrix-015');
...

{
  ...
  assert.ok(help.stdout.includes('authority --first-read|--check\nagent-onboard architecture --map|--router|--facades|--partition-plan|--partition-check|--extraction-rehearsal|--extraction-check|--golden-outputs|--golden-check|--adapter-boundary|--adapter-check|--first-slice|--first-slice-check|--check'));
