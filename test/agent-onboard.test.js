...
const EXPECTED_RELEASE_LINE = 'public_readme_history_archive_split_apply_gate';
...
fullSourceTest('public claim ledger JSONL command validates and appends explicitly', () => {
  ...
  fs.writeFileSync(ledgerFile, [
    JSON.stringify({ schema: 'agent-onboard-public-claim-ledger-entry-001', event_type: 'claim_proposed', claim_status: 'proposed', claim_id: 'claim-fixture-001', work_item_id: ['P1S3M5', 'W37'].join(''), actor: 'agent-a', created_at: '2026-07-06T00:00:00.000Z' }),
    JSON.stringify({ schema: 'agent-onboard-public-claim-ledger-entry-001', event_type: 'claim_merged', claim_status: 'merged', claim_id: 'claim-fixture-001', work_item_id: ['P1S3M5', 'W37'].join(''), actor: 'agent-a', created_at: '2026-07-06T00:01:00.000Z' })
  ].join('\n') + '\n');
  ...
  const dryRun = run(['claim', '--append', '--dry-run', '--work-item-id', ['P1S3M5', 'W38'].join(''), '--actor', 'agent-b', '--event-type', 'claim_proposed', '--claim-id', 'claim-fixture-002', '--created-at', '2026-07-06T00:02:00.000Z'], { cwd: dir });
  ...
  const write = run(['claim', '--append', '--write', '--work-item-id', ['P1S3M5', 'W38'].join(''), '--actor', 'agent-b', '--event-type', 'claim_proposed', '--claim-id', 'claim-fixture-002', '--created-at', '2026-07-06T00:02:00.000Z'], { cwd: dir });
...
fullSourceTest('public claim lifecycle check detects active claim conflicts and append refuses them', () => {
  ...
  fs.mkdirSync(path.dirname(ledgerFile), { recursive: true });
  fs.writeFileSync(ledgerFile, [
    JSON.stringify({ schema: 'agent-onboard-public-claim-ledger-entry-001', event_type: 'claim_proposed', claim_status: 'proposed', claim_id: 'claim-conflict-a', work_item_id: ['P1S3M5', 'W42'].join(''), actor: 'agent-a', created_at: '2026-07-06T00:00:00.000Z' }),
    JSON.stringify({ schema: 'agent-onboard-public-claim-ledger-entry-001', event_type: 'claim_proposed', claim_status: 'proposed', claim_id: 'claim-conflict-b', work_item_id: ['P1S3M5', 'W42'].join(''), actor: 'agent-b', created_at: '2026-07-06T00:01:00.000Z' })
  ].join('\n') + '\n');
  ...
  fs.writeFileSync(ledgerFile, JSON.stringify({ schema: 'agent-onboard-public-claim-ledger-entry-001', event_type: 'claim_proposed', claim_status: 'proposed', claim_id: 'claim-active-a', work_item_id: ['P1S3M5', 'W43'].join(''), actor: 'agent-a', created_at: '2026-07-06T00:00:00.000Z' }) + '\n');
  const rejected = run(['claim', '--append', '--dry-run', '--work-item-id', ['P1S3M5', 'W43'].join(''), '--actor', 'agent-b', '--event-type', 'claim_proposed', '--claim-id', 'claim-active-b', '--created-at', '2026-07-06T00:01:00.000Z', '--now', '2026-07-06T00:02:00.000Z'], { cwd: dir });
...
fullSourceTest('full source block line 780', () => {
  ...
  const output = readJsonOutput(result);
  assert.strictEqual(output.status, 'ok');
  assert.strictEqual(output.schema, 'agent-onboard-public-release-check-result-018');
  assert.strictEqual(output.version, EXPECTED_VERSION);
...
fullSourceTest('full source block line 2233', () => {
  ...
  const readmeHistorySurface = `${readme}\n${releaseHistory}`;
  assert.ok(readmeHistorySurface.includes('work-items --claim --write --id <public-work-item-id> --actor <actor>'));
  assert.ok(readmeHistorySurface.includes('work-items --close --write --id <public-work-item-id> --actor <actor> --summary <summary>'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target doctor --text'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target profile --text'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard work-items --summary [.agent-onboard/work-items.json]'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard work-items --summary [.agent-onboard/work-items.json] --text'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard work-items --next [.agent-onboard/work-items.json]'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard work-items --next [.agent-onboard/work-items.json] --text'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard work-items --mine [.agent-onboard/work-items.json] --actor <actor>'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard work-items --mine [.agent-onboard/work-items.json] --actor <actor> --text'));
  assert.ok(!readmeHistorySurface.includes('This release does not add claim write'));
  assert.ok(readmeHistorySurface.includes('`0.0.11` adds public `work-items --claim --dry-run`'));
  assert.ok(readmeHistorySurface.includes('`0.0.13` adds source self-dogfood and agent participation support'));
  assert.ok(readmeHistorySurface.includes('`0.0.14` adds the public source participation lifecycle gate'));
  assert.ok(readmeHistorySurface.includes('`0.0.15` adds the public handoff and closure evidence gate'));

  assert.ok(readmeHistorySurface.includes('`0.0.16` aligns public source closure tests'));
  assert.ok(readmeHistorySurface.includes('`0.0.17` adds public `release --plan` and `release --check`'));
  assert.ok(readmeHistorySurface.includes('`0.0.18` integrates that release surface into a normalized public release contract'));
  assert.ok(readmeHistorySurface.includes('`0.0.19` adds a public package contract fixture matrix'));
  assert.ok(readmeHistorySurface.includes('`0.0.20` adds installed package parity smoke'));
  assert.ok(readmeHistorySurface.includes('`0.0.21` adds the public target onboarding surface plan'));
  assert.ok(readmeHistorySurface.includes('`0.0.22` adds the public target onboarding dry-run fixture matrix'));
  assert.ok(readmeHistorySurface.includes('`0.0.23` adds the public target onboarding explicit write boundary'));
  assert.ok(readmeHistorySurface.includes('`0.0.24` adds the public target onboarding installed-package smoke'));
  assert.ok(readmeHistorySurface.includes('`0.0.25` adds the public target onboarding post-publish verification handoff'));
  assert.ok(readmeHistorySurface.includes('`0.0.26` adds the public target onboarding published package acceptance gate'));
  assert.ok(readmeHistorySurface.includes('`0.0.29` adds the public command router boundary gate'));
  assert.ok(readmeHistorySurface.includes('`0.0.30` adds the public domain service facade gate'));
  assert.ok(readmeHistorySurface.includes('`0.0.31` adds the public authority first-read index gate'));
  assert.ok(readmeHistorySurface.includes('`0.0.32` adds the public target runtime namespace gate'));
  assert.ok(readmeHistorySurface.includes('`0.0.33` adds the public package surface preservation gate'));
  assert.ok(readmeHistorySurface.includes('`0.0.34` adds the public installed parity architecture smoke gate'));
  assert.ok(readmeHistorySurface.includes('`0.0.35` adds the public source domain module partition planning gate'));
  assert.ok(readmeHistorySurface.includes('This release adds the public target repo doctor command'));
  assert.ok(readmeHistorySurface.includes('This release adds the public target onboarding repair command'));
  assert.ok(readmeHistorySurface.includes('This release adds the public target repo profile command'));
  assert.ok(readmeHistorySurface.includes('This release adds the public source module extraction adapter boundary gate'));
  assert.ok(readmeHistorySurface.includes('This release adds the public source extraction golden output freeze gate'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --golden-outputs'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --golden-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --adapter-boundary'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --adapter-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --first-slice'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --first-slice-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --bundle-parity'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --bundle-parity-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --runtime-bridge'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --runtime-bridge-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --installed-fallback-smoke'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --installed-fallback-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --second-slice-plan'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --second-slice-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --second-slice-first-slice'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --second-slice-first-slice-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --authority-bundle-parity'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --authority-bundle-parity-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --authority-runtime-bridge'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --authority-runtime-bridge-check'));
  assert.ok(readmeHistorySurface.includes('This release adds the public source module extraction installed fallback smoke gate'));
  assert.ok(readmeHistorySurface.includes('This release adds the public source module extraction runtime bridge gate'));
  assert.ok(readmeHistorySurface.includes('This release adds the public source module extraction authority runtime bridge gate'));
  assert.ok(readmeHistorySurface.includes('This release adds the public source module extraction first slice gate'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --version-sprawl-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --partition-plan'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --partition-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --extraction-rehearsal'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard architecture --extraction-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --surface'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --surface-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --source-manifest'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --source-manifest-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard authority --first-read'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard authority --check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard authority --index'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard authority --index-check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard authority --state'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard authority --state-check'));
  assert.ok(readmeHistorySurface.includes('Current release: `release --authority-state-parity-check` verifies that authority state shards remain source-only'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target doctor --json'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target profile --json'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target work-items --preview'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target handoff --preview'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target repair --plan'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target repair --write'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target metadata --plan'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target metadata --check'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target metadata --write'));
  assert.ok(readmeHistorySurface.includes('Generate `SOURCE_OF_TRUTH.md`, `authority-map.json`, and `manifest.json`'));
  assert.ok(readmeHistorySurface.includes('A target policy can override `manifest_schema`, `authority_map_schema`, `urn_namespace`'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target onboarding --write'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target onboarding --fixture'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target onboarding --trial'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard target onboarding --plan'));
  assert.ok(readmeHistorySurface.includes('The plan reports the target identity inferred from the current directory'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --parity-smoke'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --architecture-parity-smoke'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --target-onboarding-smoke'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --post-publish-handoff'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --published-acceptance'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --real-target-trial'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --contract'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --fixture'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard release --check'));
  assert.ok(readmeHistorySurface.includes('The check validates package metadata, bin entrypoints, the projected npm pack allowlist'));
  assert.ok(readmeHistorySurface.includes('Historical source-extraction release checks are reported as retired'));
  assert.ok(readmeHistorySurface.includes('The parity smoke checks that the source candidate release check passes'));
  assert.ok(readmeHistorySurface.includes('The architecture parity smoke now validates the reduced legacy architecture surface'));
  assert.ok(readmeHistorySurface.includes('The target onboarding smoke creates and removes a temporary target repo'));
  assert.ok(readmeHistorySurface.includes('The post-publish handoff emits the version-pinned npm view and npx commands'));
  assert.ok(readmeHistorySurface.includes('The published acceptance command composes release check'));
  assert.ok(readmeHistorySurface.includes('The real target trial command runs a no-write onboarding readiness check'));
  assert.ok(readmeHistorySurface.includes('source work-item ledger when that ledger is present'));
  assert.ok(readmeHistorySurface.includes('The claim response also returns `next_steps`'));
  assert.ok(readmeHistorySurface.includes('The close command reads the existing ledger'));
  assert.ok(readmeHistorySurface.includes('This release adds public work-item usability JSON views'));
  assert.ok(readmeHistorySurface.includes('This release adds public human-readable output mode'));
  assert.ok(readmeHistorySurface.includes('This release expands the public target metadata engine with policy-driven output'));
  assert.ok(readmeHistorySurface.includes('This release reduces the retired M3 architecture checker surface'));
  assert.ok(readmeHistorySurface.includes('This release extracts public runtime contracts into `cli/agent_onboard/runtime-contracts.js`'));
  assert.ok(readmeHistorySurface.includes('This release expands the public runtime contracts with shared top-level command aliases'));
  assert.ok(readmeHistorySurface.includes('`work-items --summary` returns total counts'));
  assert.ok(readmeHistorySurface.includes('`work-items --next` returns the first open item'));
  assert.ok(readmeHistorySurface.includes('`work-items --mine` returns the actor'));
  assert.ok(readmeHistorySurface.includes('Use `--text` on target-facing inspection commands'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard check --plan --text'));
  assert.ok(readmeHistorySurface.includes('npx agent-onboard check --fast --text'));
  assert.ok(readmeHistorySurface.includes('The current release adds `contracts --json|--text|--check` as a compact public contract/interface spine'));
  assert.ok(readmeHistorySurface.includes('The previous release added `target handoff --readiness-check --json|--text` as a no-write machine-readable readiness gate'));
  assert.ok(readmeHistorySurface.includes('without exporting the source-only implementation archive or requiring TypeScript/abstract classes'));
  assert.ok(readmeHistorySurface.includes('The previous release added stable handoff readiness reason codes to `target handoff --json|--text`'));
  assert.ok(readmeHistorySurface.includes('The earlier governance budget gate remains available through `target governance --budget-check --json|--text`'));
  assert.ok(readmeHistorySurface.includes('The previous release added `target governance --budget-check --json|--text` as a compact no-write target scan'));
  assert.ok(readmeHistorySurface.includes('4096-byte per-index and 8192-byte combined governance budget'));
  assert.ok(readmeHistorySurface.includes('The previous release wires governance index drift into first-read surfaces'));
  assert.ok(readmeHistorySurface.includes('`check --fast --json|--text` emits governance stale-read advisories when stored indexes are `stale`, `missing`, or blocked'));
  assert.ok(readmeHistorySurface.includes('target governance budget contract'));
  assert.ok(readmeHistorySurface.includes('The previous release added the public target governance index drift check gate'));
  assert.ok(readmeHistorySurface.includes('`target governance --check` compares stored governance indexes with freshly derived payloads and reports `fresh`, `stale`, or `missing` without writing files'));
  assert.ok(readmeHistorySurface.includes('The previous release added the public target governance index refresh integration gate'));
  assert.ok(readmeHistorySurface.includes('`work-items --init|--append|--claim|--close --write` now refreshes allowlisted governance indexes after a successful canonical `.agent-onboard/work-items.json` mutation'));
  assert.ok(readmeHistorySurface.includes('The previous release added the public target governance index explicit write product surface'));
  assert.ok(readmeHistorySurface.includes('The previous release added the public target governance index materialization dry-run product surface'));
  assert.ok(readmeHistorySurface.includes('The previous release added the public target governance preview product surface'));
  assert.ok(readmeHistorySurface.includes('The previous release added the public target handoff preview product surface'));
  assert.ok(readmeHistorySurface.includes('The previous release added the public target work-items preview product surface'));
  assert.ok(readmeHistorySurface.includes('without dependency installation, managed-project command execution, Git mutation, network access, or writes'));
  assert.ok(readmeHistorySurface.includes('The previous release added the public MCP bridge plan / skeleton product surface'));
  assert.ok(readmeHistorySurface.includes('without starting an MCP server, adding MCP dependencies, opening sockets, starting stdio transport, writing files, network, Git mutation, or publish'));
});
...
fullSourceTest('public README first-read history split planning tolerates applied archive state', () => {
  ...
  assert.strictEqual(plan.current.future_history_index_present, true);
  assert.strictEqual(plan.current.current_gate_performs_split, true);
  ...
  assert.strictEqual(check.validated.history_archive_state_valid, true);
  assert.strictEqual(check.validated.no_archive_delete_move_or_rewrite, true);
  assert.strictEqual(check.validated.current_work_item_closed, true);
  assert.strictEqual(check.boundary.check_command_writes_files, false);
});

fullSourceTest('public README history archive split dry-run replays applied state read-only', () => {
  ...
  assert.strictEqual(dryRun.mode, 'applied_replay');
  ...
  assert.strictEqual(dryRun.current.live_history_section_count, 0);
  ...
  assert.strictEqual(check.validated.applied_archive_present, true);
  assert.strictEqual(check.validated.applied_index_present, true);
  assert.strictEqual(check.validated.applied_replay_or_future_files_absent, true);
...
fullSourceTest('public README history archive split apply is verified after compaction write', () => {
  ...
  assert.strictEqual(apply.current.readme_pointer_present, true);
  assert.strictEqual(apply.current.live_history_section_count, 0);
  assert.ok(apply.current.archive_section_count >= 5);
  assert.ok(apply.current.readme_file_id.startsWith('ni:///sha-256;'));
  assert.ok(apply.current.archive_file_id.startsWith('ni:///sha-256;'));
  ...
  assert.strictEqual(check.validated.index_present, true);
  assert.strictEqual(check.validated.live_readme_pointer_present, true);
  assert.strictEqual(check.validated.live_history_sections_removed, true);
  assert.strictEqual(check.validated.archive_sections_retained, true);
