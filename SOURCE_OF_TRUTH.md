...
`P1S3M6` is the current public clean and compaction milestone. `agent-onboard release --clean-inventory`/`--clean-check` provide the read-only baseline; `release --clean-catalog`/`--clean-catalog-check` classify candidate surfaces before writes; `release --keyword-taxonomy`/`--keyword-taxonomy-check` verify the admitted package keyword taxonomy compaction. The README first-read/history split is now applied: `README.md` remains the live install, quickstart, current command, and safety-boundary surface; `docs/release-history.md` preserves historical release prose; `.agent-onboard/readme-history.index.json` records source ranges and content digests. `release --readme-plan-check`, `release --readme-dry-run-check`, and `release --readme-apply-check` validate that applied state without deleting, moving unrelated files, extracting source modules, publishing, mutating registry state, or authorizing unrelated cleanup. Any future compaction write requires a separate admitted work item, exact catalog surface id, preserved recovery/replay path, and explicit owner authorization.
